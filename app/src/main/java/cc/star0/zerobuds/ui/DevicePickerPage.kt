...
fun DevicePickerPage(onDeviceSelected: (BluetoothDevice) -> Unit) {
    ...
    if (!hasPermission) {
        Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                Text(str("bt_permission_required"))
                Spacer(Modifier.height(16.dp))
                TextButton(
                    text = str("grant_permission"),
    ...
    val pairedDevices = remember(hasPermission) {
        adapter?.bondedDevices?.toList()?.sortedByDescending {
            it.name?.contains("zero", ignoreCase = true) == true
        } ?: emptyList()
    }
    ...
    Column(Modifier.fillMaxSize()) {
        LazyColumn(
            ...
            items(pairedDevices, key = { it.address }) { device ->
                BasicComponent(
                    title = device.name ?: str("unknown_device"),
